 ...
 *
 * $Id: GameQ.php,v 1.9 2007/10/11 13:45:35 tombuskens Exp $  
 */
...
class GameQ
{
    ...
    public function addServer($id, $server)
    {
        ...
        if (!is_array($server) or count($server) < 2) throw new GameQ_ArgumentException($id);
        ...
        if ($raddr === false) throw new GameQ_ServerAddressException($id);
    ...
    /**
     * Set an output filter. Will throw a GameQ_InvalidFilterException
     * if the filter class cannot be initialized.
     *
    ...
    public function setFilter($name, $params = array())
    {
        ...
        if (!is_readable($file)) throw new GameQ_InvalidFilterException($name);
        include_once($file);

        // Check if the class can be loaded
        if (!class_exists($class)) throw new GameQ_InvalidFilterException($name);
    ...
    public function requestData()
    {
        ...
        $max_sock = $this->getOption('sockets');
    ...
    private function getProtocol($name)
    {
        ...
        // Initialize the protocol object
        if (!is_readable($file)) throw new GameQ_InvalidProtocolException($name);
        include_once($file);

        // Check if the class can be loaded
        if (!class_exists($class)) throw new GameQ_InvalidProtocolException($name);
    ...
    private function processResponse($protname, $packetname, $data)
    {
        ...
        if (!is_callable($call)) throw new GameQ_InvalidParserException($protname);
