 ...
 *
 * $Id: Config.php,v 1.3 2007/10/13 09:11:47 tombuskens Exp $  
 */
...
class GameQ_Config
{
    ...
    private function readIniFile($path)
    {
        ...
        if (count($data) == 0) {
            throw new GameQ_InvalidConfigException($path);
        }
    ...
    public function getGame($gid, $addr, $port)
    {
        ...
        if (!array_key_exists($gid, $this->games)) {
            throw new GameQ_InvalidGameException($gid);
        }
        $game = $this->games[$gid];
    ...
    public function getPackets($pid)
    {
        if (!array_key_exists($pid, $this->packets)) {
            throw new GameQ_InvalidPacketException($pid);
        }
